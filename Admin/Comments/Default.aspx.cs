...
namespace KargahProject.Admin.Comments
{
    public partial class Default : System.Web.UI.Page
    {
        ...
        {
            var Comments = CommentManager.GetUnreadComments();
        ...
        public void GenerateTable(List<Comment> Comments)
        {
            ...
            if (Comments != null)
            {
                foreach (var item in Comments)
                {
                    ...
                    btn1.Attributes["onclick"] = "ToggleCommentReaded(" + item.Id + " ,true)";
                    if (!item.IsReaded)
                    {
                    ...
                    btn2.Attributes["onclick"] = "ToggleCommentApproved(" + item.Id + " ,true)";
                    if (!item.IsApproved)
                    {
        ...
        /// <returns></returns>
        [WebMethod]
        public static bool ToggleCommentApproved(int? CommentId)
        {
            if (CommentId == null)
                return false;
            CommentManager CommentManager = new CommentManager();
            return CommentManager.ToggleCommentApproved((int)CommentId);
        }
        ...
        /// <returns></returns>
        [WebMethod]
        public static bool ToggleCommentReaded(int? CommentId)
        {
            if (CommentId == null)
                return false;
            CommentManager CommentManager = new CommentManager();
            return CommentManager.ToggleCommentReaded((int)CommentId);
        }
        ...
        /// <returns></returns>
        [WebMethod]
        public static object CommentDetails(int? CommentId)
        {
            if (CommentId == null)
                return null;
            CommentManager CommentManager = new CommentManager();
            var Comment = CommentManager.GetByIdWithJoins(CommentId);
            if (Comment == null)
                return null;
            var model = new
            {
                Id = Comment.Id,
                Text = Comment.Text,
                ProductTitle = Comment.ProductId != null ? "<a target='_blank' href='/Admin/Products/Details.aspx?id=" + Comment.ProductId + "'>" + Comment.ProductTitle + "<a/>" : Comment.ProductTitle,
                UserFullName = Comment.UserId == null ? Comment.FullName + "<br/> ایمیل : " + Comment.Email
                                       : "<a target='_blank' href='/Admin/Users/Details.aspx?id=" + Comment.UserId + "'>" + Comment.UserFullName + "<a/>"
            };
            return model;
        }
        ...
        public static bool DeleteComment(int? CommentId)
        {
            if (CommentId == null)
                return false;
            CommentManager CommentManager = new CommentManager();
            return CommentManager.Delete((int)CommentId);
        }
